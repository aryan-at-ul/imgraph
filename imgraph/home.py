...
def get_home_dir() -> str:
    r"""Get the cache directory used for storing all imgraph-related data.
    If :meth:`set_home_dir` is not called, the path is given by the environment
    variable :obj:`$IMGRAPH_HOME` which defaults to :obj:`"~/.cache/imgraph"`.
    """
    if _home_dir is not None:
        return _home_dir

    home_dir = os.getenv(ENV_IMGRAPH_HOME, DEFAULT_CACHE_DIR)
    home_dir = osp.expanduser(home_dir)
    return home_dir


def set_home_dir(path: str):
    r"""Set the cache directory used for storing all imgraph-related data.
    Args:
